...
quarterly_upb_to_loanamt = function(annual_interest_rate, principal, years, years_plot = years, mltv_init = 0.72) {
    ...
    sidebarPanel(
      ...
      tags$div(class = "form-group shiny-input-container",
        ...
        tags$div(class="input-group",
          tags$input(type = "text", class = "form-control", id="interest", placeholder="enter 1 for 1%", value = "3.25"),
          tags$div(class = "input-group-addon",
                   tags$span(class = "input-group-text","%")
          )
        )
      ),
      textInput("years", "Mortgage Term (Years)", value = "30", placeholder = "years"),
      textInput("years_graph", "Number of years to graph", value = "30"),
      tags$div(class = "form-group shiny-input-container",
               tags$label(class="control-label", "Starting MLTV (%)"),
               tags$div(class="input-group",
                        tags$input(type = "text", class = "form-control", id="mltv_init", placeholder="enter 1 for 1%", value = "72"),
                        tags$div(class = "input-group-addon",
                                 tags$span(class = "input-group-text","%")
                        )
               )
...
server <- function(input, output) {
  output$mltv_plot <- renderPlotly({
    ...
    if (input$option == "upb") {
      ...
      g <- ggplot(data_frame, mapping = aes(x=Quarter, y=MLTV)) +
        geom_point(size = 1, color = "#003f5c") +
