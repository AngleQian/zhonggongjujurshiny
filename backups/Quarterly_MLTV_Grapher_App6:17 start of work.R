...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      fluidRow(id="mltv_init_panel",
        column(
          width = 12,
          div(style = "white-space: nowrap", 
              div(style="display: inline-block; width: 95%;",
                  textInput("mltv_init", "Starting MLTV (%)", value = "72", placeholder = "enter 1 for 1%"),
              ),
              h5('%', style="display:inline-block")
          )
        ),
...
server <- function(input, output) {
  output$mltv_plot <- renderPlot({
    ...
    if (input$option == "upb") {
      ...
      g <- ggplot(data_frame, mapping = aes(x=Quarters, y=UPB)) +
      ...
      g <- ggplot(data_frame, mapping = aes(x=Quarters, y=MLTV)) +
        geom_point(size = 2, color = "#003f5c") +
